<script>
	...
	const validate = () => {
		myForm.validate()
		const noError= $myForm.hasError("name.required") || $myForm.hasError("password.required") || $myForm.hasError("email.required") || $myForm.hasError("passwordConfirmation.match_field")
		if(!noError){
			const user = {
				username: $name.value,
