<script>
	...
	const myForm = form(name, password, passwordConfirmation);
	function validate() {
		name.validate();
		password.validate();
	...
	(async () => {
  const rawResponse = await fetch('https://8080-danielyatali-flaskmvc-3cd8pezq48y.ws-us38.gitpod.io/signup', {
    method: 'POST',
    headers: {
